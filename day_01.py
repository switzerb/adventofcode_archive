...
def find_position(floor_input):
    result = reduce(lambda a, i: {'position': a['position'] + 1, 'sum': a['sum'] + floor(i), 'basement': (a['sum'] + floor(i)) < 0} if not a['basement'] else a, floor_input, {'position': 0, 'sum': 0, 'basement': False})
...
data = open("day_01_input.txt").read().strip()
...
suite = unittest.TestLoader().loadTestsFromTestCase(TestDayOne)
